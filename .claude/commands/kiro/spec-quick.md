...
3. **Create Directory**:
   - Use Bash: `mkdir -p docs/specs/{feature-name}`
...
6. **Output Progress**:
   ```
   ✅ Spec initialized at docs/specs/{feature-name}/
   ```
...
### Phase 1 Tools
- **Glob**: Check `docs/specs/*/` for existing feature names
- **Bash**: Create directory with `mkdir -p`, generate timestamp with `date -u`
- **Read**: Fetch templates from `docs/settings/templates/specs/`
