---
name: steering-agent
description: Maintain docs/steering/ as persistent project memory (bootstrap/sync)
...
## Role
You are a specialized agent for maintaining `docs/steering/` as persistent project memory.
...
- For Bootstrap mode: Read templates from `docs/settings/templates/steering/`
- For Sync mode:
  - Glob(`docs/steering/*.md`) to get all existing steering files
  - Read each steering file
- Read steering principles: `docs/settings/rules/steering-principles.md`
